...
def gicp(source_points, target_points, max_iterations=60, tolerance=1e-6, epsilon=1e-6, max_distance_correspondence=150, max_distance_nearest_neighbors=50):
    ...
    for iteration in range(max_iterations):
        ...
        for i, source_point in enumerate(transformed_source_points):
            ...
            combined_cov_matrix = source_cov_matrices[i] + target_cov_matrices[index] @ np.eye(2).T + epsilon * np.eye(2)
        ...
        # Store the transformation matrix, source covariance matrices, target covariance matrices and weight matrices
        all_weight_matrices.append(weight_matrices)
        all_transformations.append(transformation_matrix)

    all_weight_matrices.append(all_weight_matrices[-1])
    return transformation_matrix, all_transformations, initial_source_cov_matrices, target_cov_matrices, all_weight_matrices
...
def visualize_icp(source_cloud, target_cloud, all_transformations, source_cov_matrices, target_cov_matrices, all_weight_matrices):
    pygame.init()
    ...
    while running:
        ...
        screen.blit(font.render("Press 'w' to toggle weight matrices", True, (255, 255, 255)), (400, 90))
        ...
        if keys[pygame.K_e]:
            ...
            pygame.time.wait(100)
        if keys[pygame.K_w]:
            show_weight_matrices = not show_weight_matrices
            pygame.time.wait(100)
        ...
        if show_weight_matrices:
            for i, point in enumerate(transformed_source):
                draw_ellipse(screen, purple, point, all_weight_matrices[step][i], n_std=100, alpha=100)
...
if __name__ == "__main__":
    ...
    T, all_transformations, source_cov_matrices, target_cov_matrices, all_weight_matrices = gicp(source_points, target_points)

    visualize_icp(source_points, target_points, all_transformations, source_cov_matrices, target_cov_matrices, all_weight_matrices)
